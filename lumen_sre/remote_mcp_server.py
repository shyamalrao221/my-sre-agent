...
class RemoteToolHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if route == "/tools/cost-analysis":
            ...
            self._write_json({"result": fetch_cost_optimization_snapshot(namespace=namespace)})
            return
    ...
    def do_POST(self):
        ...
        if parsed.path == "/tools/query":
            ...
            result = asyncio.run(manager.handle_query(user_query=query, user_id=user_id))
...
def main(host: str = "127.0.0.1", port: int = 8080, open_browser_on_start: bool = True):
    ...
    print(f"Remote SRE tool server listening on {app_url}")
